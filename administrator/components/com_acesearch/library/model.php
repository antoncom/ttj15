...
class AcesearchModel extends JModel {
	...
	function _buildViewQuery() {
		...
		$this->_query = "SELECT * FROM #__acesearch_{$this->_table} {$where}{$orderby}";
	}
	...
	function getTotal() {
		if (empty($this->_total)) {			
			$this->_total = AceDatabase::loadResult("SELECT COUNT(*) FROM #__acesearch_{$this->_table}".$this->_buildViewWhere());	
		}
		return $this->_total;
	}
