...
// Model Class
class AcesearchModelUpgrade extends AcesearchModel {
    ...
    function upgrade() {
		...
		if ($type == 'upload') {
			...
			$url = self::_getURL();
			$package = AcesearchInstaller::getPackageFromServer($url);
		}
		...
		// Get current version
		$cur_version = AcesearchUtility::getXmlText(JPATH_ACESEARCH_ADMIN.DS.'acesearch.xml', 'version');
		...
		if ($this->_fileError == false) {
			...
			if (!$errors) {
				JFactory::getApplication()->enqueueMessage(JText::_('COM_ACESEARCH_UPGRADE_SUCCESS'));
			}
			else {
				JError::raiseWarning(100, JText::_('COM_ACESEARCH_UPGRADE_UNSUCCESS'));
			}
		}
	...
	function _getURL() {
		$f = "dow"."nloa"."ds/ac"."e"."se"."ar"."ch/co"."mpo"."nent/ac"."ese"."arc"."h-ba"."si"."c/d"."ow"."nlo"."ad?method=upgrade";
		$c = "do"."wnl"."oad"."s/dow"."nlo"."ad-r"."equ"."est?do"."wnl"."oa"."d_i"."d=";
		$download_id = $this->AcesearchConfig->download_id;
