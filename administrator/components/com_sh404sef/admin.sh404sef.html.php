 ...
 * @package     sh404SEF-15
 * @version     $Id: admin.sh404sef.html.php 852 2009-01-02 21:42:53Z silianacom-svn $
 *
...
<table class="adminlist">
	...
	<tr class="row<?php echo $k; ?>">
		...
		if( $viewmode == 1 ) {
			...
			onclick="return listItemTask('cb<?php echo $i;?>','edit')"> <?php echo $row->oldurl;?>
		</a> <?php
		} ?></td>
	...
	function viewDuplicates( &$rows, &$lists, $pageNav, $option, $id ) {
		JHTML::_('behavior.tooltip');
		JToolBarHelper::title( _COM_SEF_MANAGE_DUPLICATES.$rows[0]->oldurl, 'cpanel.png' );
		?>
...
<table class="adminlist">
	...
	<tr class="row<?php echo $k; ?>">
		...
		<td style="text-align: left"><a href="#edit"
			onclick="return listItemTask('cb<?php echo $i;?>','makeMainUrl')"> <?php echo htmlspecialchars($row->newurl); ?>
		</a></td>
	</tr>
	<?php
	$k = 1 - $k;
...
<table class="adminlist">
	<tr>
		...
		<td><input class="inputbox" type="text" size="100" name="oldurl"
			value="<?php echo $_row->oldurl; ?>" /> <?php echo JHTML::_('tooltip',_COM_SEF_TT_OLDURL); ?>
		</td>
	</tr>
	<tr>
		...
		if( ($_row->id) && $_row->dateadd == '0000-00-00' ) {
			...
			value="<?php echo htmlspecialchars( $_row->newurl ); ?>" /> <?php echo JHTML::_('tooltip', _COM_SEF_TT_NEWURL );
		} ?></td>
	</tr>
	<tr>
...
<table class="adminlist">
	...
	<tr>
		...
		<td><?php echo JHTML::_('grid.id', $i, $row->id, false ); ?></td>
		<td style="text-align: left;"><a href="#edit"
			onclick="return listItemTask('cb<?php echo $i; ?>','edit')"> <?php echo htmlspecialchars( $row->newurl ); ?>
		</a></td>
	</tr>
	<?php
	} ?>
	...
	function editMeta( &$_row, $_option, $returnTo, $editUrl, $oldUrl ) {
		JHTML::_('behavior.tooltip');
		JToolBarHelper::title( ($_row->id ? _COM_SEF_META_EDIT : _COM_SEF_META_ADD).(empty($oldUrl) ? '': ' : '.$oldUrl),
			'addedit.png' );
		?>
<script type="text/javascript">
			/* <![CDATA[ */
...
if( !empty( $editUrl ) ) {  ?>
	<tr>
		...
		<td><input class="inputbox" type="text" size="100" name="newurl"
			value="<?php echo htmlspecialchars($_row->newurl); ?>" /> <?php echo JHTML::_('tooltip',_COM_SEF_TT_NEWURL_META); ?>
		</td>
	</tr>
	<?php
} ?>
