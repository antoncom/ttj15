...
(function($) {
	...
	var Meta = {};
	Meta.pos  = $.extend(["top", "bottom", "left", "right"], {
		camel: ["Top", "Bottom", "Left", "Right"]
	});
	Meta.size = $.extend(["height", "width"], {
		camel: ["Height", "Width"]
	});
		...
		return this.each(function() {
			...
			$target.data("onTimer", setTimeout(function() {
				if(options.showAnimation) {
					...
					// NOTE set focus for input element in balloon
					if ($("input", $balloon).length > 0) {
						$("input", $balloon).focus().select();						
					}
				}
			}, options.delay));
		});
