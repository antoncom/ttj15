...
class Calendar_EventIterator implements Iterator {
	...
	function __construct($start, $end, $repeat_mode = "weekly", $repeat_interval = 1) {
		$this->start = $start;
		$this->end = $end;
		...
		$this->is_month_last_day = date('t', strtotime($this->start)) == date('j', strtotime($this->start));
	}
	...
	function next() {
		...
		if ($this->repeat_str == "month" && $this->is_month_last_day) {
			...
			$date = mktime(
					date("H", $date), date("i", $date), date("s", $date), date("m", $date) + $this->direction, 1,
					date("Y", $date));
