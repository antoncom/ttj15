<script>
	...
	function check_repeat_event(todo_id, f_repeat, f_norepeat) {
		...
		$.get(TeamTime.baseUrl +
			"index.php?option=com_teamtime&controller=todo&task=check_repeat_event"+
			"&todo_id=" + todo_id,
		function (data) {
			if (data == "1") {
				f_repeat();
		...
		function(){
			$("#all_repeat_todo").click(function(){
				$.post(TeamTime.baseUrl +
					"index.php?option=com_teamtime&controller=todo&task=remove_all_repeat_todo"+
					"&id=" + todo_id +
			...
			$("#rest_repeat_todo").click(function(){
				$.post(TeamTime.baseUrl +
					"index.php?option=com_teamtime&controller=todo&task=remove_rest_repeat_todo"+
					"&id=" + todo_id +
					"&current_date=" + d, ondelete);
				$.alerts._hide();
			});
			...
			$("#only_repeat_todo").click(function(){
				$.post(TeamTime.baseUrl +
					"index.php?option=com_teamtime&controller=todo&task=remove_only_repeat_todo"+
					"&id=" + todo_id +
					"&current_date=" + d, ondelete);
				$.alerts._hide();
			});
	...
	function edit_repeat_events_box(todo_id, current_date, onedit) {
		...
		function(){
			...
			$("#rest_repeat_todo").click(function(){
				$.post(TeamTime.baseUrl +
					"index.php?option=com_teamtime&controller=todo&task=edit_rest_repeat_todo"+
					"&id=" + todo_id +
			...
			$("#only_repeat_todo").click(function(){
				$.post(TeamTime.baseUrl +
					"index.php?option=com_teamtime&controller=todo&task=edit_only_repeat_todo"+
					"&id=" + todo_id +
					"&current_date=" + d, function(data){
					var d = [current_date.getMonth() + 1, current_date.getDate(), current_date.getFullYear()].join("/");
					$("#stpartdate").val(d);
	...
	function exclude_repeat_events_box(todo_id, new_date, old_date, onmove) {
		...
		function(confrm_result){
			...
			if (confrm_result) {
				$.post(TeamTime.baseUrl +
					"index.php?option=com_teamtime&controller=todo&task=exclude_repeat_todo",
