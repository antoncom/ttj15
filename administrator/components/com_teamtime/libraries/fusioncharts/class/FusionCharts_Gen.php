...
##------------ PUBLIC FUNCTIONS ----------------------------------------------------------------
  ...
  function renderChart($js_code = false) {

    $this->strXML = $this->getXML();
    $this->SWFFile = $this->SWFPath . "FCF_" . $this->chartSWF[$this->chartType][0] . ".swf";
    # print the charts
    if ($js_code)
      print renderChartJS($this->SWFFile . $this->chartMSG, "", $this->strXML, $this->chartID, $this->width, $this->height, $this->debugMode, $this->registerWithJS, $this->bgColor, $this->scaleMode, $this->lang);
    else
...
##------------ PRIVATE FUNCTIONS ----------------------------------------------------------------
  ...
  function encodeSpecialChars($strValue) {
    $strValue = str_replace("&", "%26amp;", $strValue);
