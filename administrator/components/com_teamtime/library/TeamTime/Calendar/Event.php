...
class TeamTime_Calendar_Event {
	...
	function generate_dates() {
		...
		if (sizeof($days) == 0) {
			$it = new TeamTime_Calendar_EventDateFilter(
							new TeamTime_Calendar_EventIterator($this->data->created, $this->data->end_date,
			...
			$it = new TeamTime_Calendar_EventDateFilter(
							new TeamTime_Calendar_EventIterator($this->data->created, $this->data->start_date,
									$this->data->repeat_mode, $this->data->repeat_interval),
							$this->data->start_date, $this->data->end_date);
			foreach ($it as $key => $value) {
				$result[] = $value;
			...
			$it = new TeamTime_Calendar_EventDateFilter(
							new TeamTime_Calendar_EventWeekIterator($this->data->created, $this->data->end_date,
			...
			$it = new TeamTime_Calendar_EventDateFilter(
							new TeamTime_Calendar_EventWeekIterator($this->data->created, $this->data->start_date,
									$this->data->repeat_mode, $this->data->repeat_interval, $days),
							$this->data->start_date, $this->data->end_date);
			foreach ($it as $key => $value) {
				$result[] = $value;
