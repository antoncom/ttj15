...
class WuserViewWusers extends JView {
	...
	function display($tpl = null) {
		...
		$date_presets['last_month'] = array(
			...
			'until' => date('Y-m-d',
					mktime(0, 0, 0, date('n', $date), 0, date('Y', $date))));
		...
		$date_presets['month'] = array(
			'name' => 'This Month',
			'from' => date('Y-m-d',
					mktime(0, 0, 0, date('n', $date), 1, date('Y', $date))),
			'until' => date('Y-m-d',
					mktime(0, 0, 0, date('n', $date) + 1, 0, date('Y', $date))));
		...
		$date_presets['next_month'] = array(
			...
			'from' => date('Y-m-d',
					mktime(0, 0, 0, date('n', $date) + 1, 1, date('Y', $date))),
			'until' => date('Y-m-d',
					mktime(0, 0, 0, date('n', $date) + 2, 0, date('Y', $date))));
		...
		if ($from->_date === false || $until->_date === false) {
			if ($from_period != '?' && $until_period != '?') {
				JError::raiseNotice(500,
						JText::_('Please enter a valid date format (YYYY-MM-DD)'));
			}
		...
		$options = array(JHTML::_('select.option', '',
					'- ' . JText::_('Select Period') . ' -', 'text', 'value'));
		foreach ($date_presets as $datename => $value) {
			$options[] = JHTML::_('select.option', $datename, JText::_($value['name']),
							'text', 'value');
		...
		$lists['select_date'] = JHTML::_(
						'select.genericlist', $options, 'period', 'class="inputbox" size="1"',
						'text', 'value', $select);
		...
		$search = $mainframe->getUserStateFromRequest($option . $name . '.search',
				'search', '', 'string');
		...
		// init hours_fact
		if ($from_period && $until_period) {
			$where_hours_fact = ' and date >= ' . $db->Quote($db->getEscaped("$from_period 00:00:00",
									true), false) .
					' and date <= ' . $db->Quote($db->getEscaped("$until_period 23:59:59", true),
							false);
		}
		else {
			$where_hours_fact = "";
		...
		if ($from_period && $until_period) {
			$where_hours_fact2 = ' and created >= ' . $db->Quote($db->getEscaped("$from_period 00:00:00",
									true), false) .
					' and created <= ' . $db->Quote($db->getEscaped("$until_period 23:59:59",
									true), false);
		}
		else {
			$where_hours_fact2 = "";
