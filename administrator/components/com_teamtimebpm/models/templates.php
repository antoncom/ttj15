...
class TeamtimebpmModelTemplates extends Core_Joomla_ManagerList {
	...
	public function getData() {
		...
		if (empty($this->_data)) {
			...
			foreach ($this->_data["Untagged"] as $i => $item) {
				$this->_data["Untagged"][$i]->tags = $mTemplate->getTags($item->tags);
			}
		}
			...
			// group by spaces
				...
				foreach ($mSpace->getOptionsList() as $group) {
					...
					foreach ($this->_data[$group->text] as $i => $item) {
						$this->_data[$group->text][$i]->tags = $mTemplate->getTags($item->tags);
					}
				}
			}
		}
