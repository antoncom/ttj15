...
(function ($) {
	...
	$.fn.bcalendar = function(option) {
		...
		var __SCOLLEVENTTEMP = "<DIV style=\"WIDTH:${width};top:${top};left:${left};\" title=\"${title}\" class=\"chip chip${i} ${drag}\"><div class=\"dhdV\" style=\"display:none\">${data}</div><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}\" class=ct>&nbsp;</DIV><DL style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; HEIGHT: ${height}px; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\"><DT style=\"BACKGROUND-COLOR:${bgcolor2}\">${plantime} ${icon} ${expenses}</DT><DD><SPAN>${content}</SPAN></DD><DIV class='resizer' style='display:${redisplay}'><DIV class=rszr_icon>&nbsp;</DIV></DIV></DL><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; BORDER-RIGHT-COLOR: ${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb1>&nbsp;</DIV><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BORDER-TOP-COLOR:${bdcolor}; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb2>&nbsp;</DIV></DIV>";
		var __ALLDAYEVENTTEMP =
		'<div class="rb-o ${eclass}" id="${id}" title="${title}" style="color:${color};">' +
		'<div class="dhdV" style="display:none">${data}</div>' +
		'<div class="${extendClass} rb-m" style="background-color:${color}">${extendHTML}' +
		'<div class="rb-i">${content}</div></div></div>';
		...
		function render() {
			...
			for(var i = 0; i < events.length; i++){
				...
				var t = parseInt(events[i][0]);
				var is_new = !(t in tmp_events) ||
				((t in tmp_events) && (tmp_events[t][2].getTime() != events[i][2].getTime()));
				if(is_new){
					tmp_events_data.push(events[i]);
					tmp_events[t] = events[i];
				}
			}
			events = tmp_events_data;
			...
			switch (option.view) {
				case "day":
					...
					// for remove scrolling
					$("#dvCalMain").height($("#dvtec").height() + 40);
					break;
				...
				case "month":
					...
					// for remove scrolling
					$("#dvCalMain").height($("#mvEventContainer").height() + 15);
					break;
		...
		function BuildDaysAndWeekView(startday, l, events, config) {
			...
			// for remove scrolling
			html.push("<div id=\"dvtec\"  class=\"scolltimeevent\"><table style=\"table-layout: fixed;", 
				$.browser.msie ? "" : "width:100%", "\" cellspacing=\"0\" cellpadding=\"0\"><tbody><tr><td>");
		...
		function escapeQuot(s) {
			return s.replace(/'/g, "&lsquo;").replace(/"/g, "&quot;");
		}
		...
		function getTitle(event) {
			...
			ret.push(escapeQuot(eventshow));
			ret.push(i18n.xgcalendar.location + ": " + escapeQuot(locationshow));
			ret.push(i18n.xgcalendar.participant + ": " + escapeQuot(attendsshow));
		...
		function BuildDayEvent(theme, e, index) {
			...
			p.plantime = e.event[11];
			p.expenses = e.event[12];
		...
		function BuildMonthDayEvent(e, cday, length) {
			...
			var cen;
			if (!e.allday && !sf) {
				// no print hours and minutes in month view
		...
		function populate() {
			...
			if (option.url && option.url != "") {
				...
				{
					name: "showdate",
					value: dateFormat.call(option.showday, "M/d/yyyy")
				},
		...
		function dayshow(e, data) {
			...
			if (data != null) {
				if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
					var csbuddle =
					'<div id="bbit-cs-buddle" style="z-index: 180; width: 420px;visibility:hidden;" class="bubble"><table class="bubble-table" cellSpacing="0" cellPadding="0"><tbody><tr><td class="bubble-cell-side"><div id="tl1" class="bubble-corner"><div class="bubble-sprite bubble-tl"></div></div><td class="bubble-cell-main"><div class="bubble-top"></div><td class="bubble-cell-side"><div id="tr1" class="bubble-corner"><div class="bubble-sprite bubble-tr"></div></div>  <tr><td class="bubble-mid" colSpan="3"><div style="overflow: hidden" id="bubbleContent1"><div><div></div><div class="cb-root"><table class="cb-table" cellSpacing="0" cellPadding="0"><tbody><tr><td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><div id="bbit-cs-what" title="'
					+ i18n.xgcalendar.click_to_detail + '" class="textbox-fill-div lk" style="cursor:pointer;"></div></div></div></td></tr><tr><td class=cb-value><div id="bbit-cs-buddle-timeshow"></div></td></tr></tbody></table>'

					+ '<div class="bbit-cs-split" style="white-space:nowrap"><input id="bbit-cs-id" type="hidden" value=""/>'
					+ '<div id="bbit-cs-delete" class="lk">[' + i18n.xgcalendar.i_delete + ']</div>'
					//+ ' <SPAN id="bbit-cs-editLink" class="lk">'
					//+ i18n.xgcalendar.update_detail + ' </SPAN>&nbsp; '
					+ '<div id="bbit-cs-treeLink" class="lk"></div>'
					+ '<div id="bbit-cs-treeHours"></div>'
					+ '</div>'
					+ '<div id="bbit-cs-treeContent"></div>'
					...
					// clear bubble box
					$("#bbit-cs-buddle-timeshow").html("");
					$("#bbit-cs-treeContent").html("");
					$("#bbit-cs-treeLink").html("");
					$("#bbit-cs-treeLink").css({
						"font-weight": "normal"
					});
					$("#bbit-cs-buddle").css({
						"width": "420px"
					});
					$("#bbit-cs-treeContent").css({
						"border-top": "none"
					});
					$("#prong1 div.bubble-sprite").show();
				...
				$.get(TeamTime.baseUrl +	
					"index.php?option=com_teamtimecalendar&controller=calendar&task=todo_info" + 
					"&id="+data[0],
					function (data) {
						$("#bbit-cs-buddle-timeshow").html(data);
						$("#bbit-cs-buddle-timeshow a.calendar-process-link").click(
							function () {
				...
				// load todo tree info
				$.get(TeamTime.baseUrl +
					"index.php?option=com_teamtimecalendar&controller=calendar&task=todo_subtree" +
					"&id=" + data[0] +
					"&date=" + dateFormat.call(data[2], "yyyy-M-d") +
					"&view=" + option.view,
					function (resp_data) {
						$("#bbit-cs-treeLink").html(resp_data[0]);
						$("#bbit-cs-treeContent").html(resp_data[1]);
						$("#bbit-cs-treeHours").html(resp_data[2]);

						$("#bbit-cs-treeLink, #bbit-cs-treeLink2").unbind('click');

						if (resp_data[1] == "") { // is not parend todo
							// link for create copy todo
							$("#bbit-cs-treeLink").click(function () {
								var data = $("#bbit-cs-buddle").data("cdata");
								data[0] = "copy_" + data[0];
							...
							// link for create copy todo
							$("#bbit-cs-treeLink2").click(function () {
								var data = $("#bbit-cs-buddle").data("cdata");
								data[0] = "copy_" + data[0];
								option.EditCmdhandler.call(this, data);
							...
							$("a.open_edit_todo").click(function () {
		...
		function moreshow(mv) {
			...
			//console.log([daystr, arrdays, day, i18n.xgcalendar.dateformat.Md3]);
		...
		function dayupdate(data, start, end) {
			if (option.quickUpdateUrl != "" && data[8] == 1 && option.readonly != true) {
				...
				var start_date = dateFormat.call(start, i18n.xgcalendar.dateformat.fulldayvalue + " HH:mm");
				var old_start_date = dateFormat.call(os, i18n.xgcalendar.dateformat.fulldayvalue + " HH:mm");
				...
				check_repeat_event(id,
					function(){
						exclude_repeat_events_box(id, start_date, old_start_date,
							function(data) {
								option.isloading = false;
							});
					},
					...
					function(){
						if (option.quickUpdateHandler && $.isFunction(option.quickUpdateHandler)) {
							option.quickUpdateHandler.call(self, param);
						}
						else {
			...
			if (buddle.length == 0) {
				...
				if (option.EditCmdhandler && $.isFunction(option.EditCmdhandler)) {
					option.EditCmdhandler.call(this,
						['0', $("#bbit-cal-what").val(), start, end, isallday]);
				}
