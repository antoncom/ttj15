...
class TeamTime_EventHandlers_Formals {
	...
	public function onInit() {
		...
		// include js libs
		JHTML::script('default.js', 'administrator/components/com_teamtimeformals/assets/js/');
		...
		require_once($path . '/models/doctype.php');
	...
	public function onSaveTodo($todoItem, $post) {
		...
		if (isset($post["clientmail"]) && $post["clientmail"] == "1") {
			...
			$doctype = $mFormal->getDoctype($params["doctype_id"]);
			$formalData["name"] = JText::_($doctype->name) . " - " . $todoItem->title;
			$res = $mFormal->generateContent($params);
			...
			if (trim($variablesData['email_from_tech_req']) != '') {
				$mailform = $variablesData['email_from_tech_req'];				
			}
			else {
				$mailform = $config->mailfrom;				
			}
			
			if (trim($variablesData['from_tech_req']) != '') {
				$mailfromname = $variablesData['from_tech_req'];
			}
			else {
				$mailfromname = $config->fromname;
			}

			//error_log('send [' . $formalData['name'] . '] from ' . $mailform . ', ' . $mailfromname);

			JUTility::sendMail($mailform, $mailfromname, $recipient, $formalData['name'],
					$formalData['content'], true);
		}
	}
	...
	public function onSaveProjectParams($model, $post) {
		...
		$projectId = $model->_data->id;
		$variablem = new VariableModelVariable();
		$variablem->setVariablesForProject($projectId, $post["params"]["variables"]);
	}
