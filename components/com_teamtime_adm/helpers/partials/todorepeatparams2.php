...
if ($event) {
	...
	if ($repeat_params) {
		...
		$tmp = explode(" ", $repeat_params->start_date);
		...
		if ($repeat_params->end_date == '0000-00-00 00:00:00')
			...
			$tmp = explode(" ", $repeat_params->end_date);
...
<div>
	...
	<div id="repeating_params">
		<table>
			<tr>
				<td><strong><?= JText::_('Repeating') ?>:</strong></td>
				<td><select size="1" name="repeating" id="repeating">
						<option value="monthly"
						<?=
						$event && $repeat_params->repeat_mode == "monthly" ? "selected" : ""
						?>
										label="<?= JText::_('Every month') ?>"><?= JText::_('Every month') ?></option>
						<option value="weekly"
						<?=
						$event && $repeat_params->repeat_mode == "weekly" ? "selected" : ""
						?>
										label="<?= JText::_('Every Week') ?>"><?= JText::_('Every Week') ?></option>
						<option value="yearly"
						<?=
						$event && $repeat_params->repeat_mode == "yearly" ? "selected" : ""
						?>
			...
			<tr>
				...
				<td nowrap nowrap="nowrap"><select size="1" name="repeat_interval" id="repeat_interval">
						...
						foreach (range(1, 24) as $i) {
							...
							<option value="<?= $i ?>"
							<?=
							$event && $repeat_params->repeat_interval == $i ? "selected" : ""
							?>
											label="<?= $i ?>"><?= $i ?></option>
										<? } ?>
			...
			<tr>
				...
				<td>
					<?php
					echo JHTML::_('calendar', $event && $repeat_params ? $repeat_params->start_date : $todo_date,
							'start_date', 'start_date', JText::_('DATE_FORMAT_MYSQL_WITHOUT_TIME'));
					?>
				</td>
			</tr>
			<tr>
				<td><strong><?= JText::_('End date') ?>:</strong></td>
				<td><table>
						<tr>
							<td nowrap nowrap="nowrap"><input type="radio" id="end_date_type0"
																								name="end_date_type"
																								<?=
																								(($event && $repeat_params->end_date == "") || !$repeat_params) ? "checked" : ""
																								?>
																								value="0" /> <?= JText::_('REPEATING END NEVER') ?>&nbsp;&nbsp;<input type="radio"
																								id="end_date_type1" name="end_date_type"
<?=
$event && $repeat_params->end_date != "" ? "checked" : ""
?>
																								value="1" /><?= JText::_('REPEATING END UNTILL') ?>:

<?php
echo JHTML::_('calendar', $event && $repeat_params ? $repeat_params->end_date : "", 'end_date',
		'end_date', JText::_('DATE_FORMAT_MYSQL_WITHOUT_TIME'));
?>
							</td>
							<td></td>
						</tr>
			...
			<tr>
				...
				<td><div id="repeating_result"><strong></strong></div></td>
			</tr>
		</table>
	</div>
</div>
	...
	(function($){
		$(document).ready(function(){
			...
			var change_repeating_params = function(){
				...
				var created_date = $("#detailscreated").val().split(" ");

				created_date = created_date[0].split("-");
