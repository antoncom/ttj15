...
class JHTMLTeamlog {
	...
	function projectList($options, $name, $attribs = null, $key = 'value', $text = 'text',
	...
	function todoPeriodList($options, $name, $attribs = null, $key = 'value', $text = 'text',
		...
		$db = & JFactory::getDBO();
		$table_todo = new TableTodo($db);
		$user = & YFactory::getUser();
		$user_id = $user->id;

		$params = array();
		if (JRequest::getVar("reset_filter") == 1) {
			$params = TeamTime::getDefaultFilter();
		}
	...
	function todoList($project_id, $current_todo_id, $options, $name, $attribs = null, $key = 'value',
		...
		$cmd = array(
				"hide_node" => array("node_id" => $current_todo_id)
		);
