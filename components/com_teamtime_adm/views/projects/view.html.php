...
/*
   Class: ProjectViewProjects
   The View Class for Projects
*/
class ProjectViewProjects extends JView {
	...
	function display($tpl = null) {
		...
		$lists['select_date'] = JHTML::_(
			'select.genericlist', $options, 'period',
		...
		foreach($items as $i=>$item){
			$sql = "select sum(duration)/60 as sfact from #__teamlog_log
				where project_id = {$item->id} $where_hours_fact";
			$db->setQuery( $sql );
			...
			$sql = "select sum(hours_plan) as splan from #__teamlog_todo
				where project_id = {$item->id} $where_hours_fact2";
			$db->setQuery( $sql );
		...
		$lists['select_state'] = JHTML::_('teamlog.projectstatelist', $options, 'filter_state', 'class="inputbox auto-submit"', 'value', 'text', $filter_state);
