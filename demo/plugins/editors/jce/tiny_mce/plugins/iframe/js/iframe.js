var IFrameDialog = {
	init : function() {
		...
		if (pl != "") {
			pl = eval(pl);
			// Special attributes
			tinymce.each(['src', 'frameborder', 'marginwidth', 'marginheight', 'scrolling', 'title', 'name', 'allowtransparency'], function(v){
				t.setStr(pl, v);																										   
			});
			// Standard attributes
	...
	serializeParameters : function() {
		...
		s += this.getStr('allowtransparency');
		s += this.getStr('title');
		s += this.getStr('name');
