...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		this.opts = $.extend({
			...
			/* customisation for multiple upload */
			'<input type="file" id="redactor_file" name="file[]" multiple />' +
			'</div>' +
			'</form>',
	...
	Redactor.prototype = {
		...
		fileUploadCallback: function(json)
		{
			...
			// var linkBuff = [RLANG.label_loaded_results];
			var linkBuff = [];
			if (data.length == 1) {
		...
		uploadInit: function(element, options)
		{
			...
			if (this.uploadOptions.auto)
			{
				$(this.uploadOptions.input).change($.proxy(function()
				{
					...
					if (this.uploadOptions.ajaxUpload) {
						this.uploadAjaxSubmit();
					}
					else {
						this.uploadSubmit();
					}
		...
		uploadAjaxSubmit: function () {
			//console.log(this.element);

			var files = $("#redactor_file")[0].files;
			var fd = new FormData();
			...
			var uploadProgress = $.proxy(function (e) {
				var dropareabox = $(".redactor_dropareabox");

				var percent = parseInt(e.loaded / e.total * 100, 10);
				dropareabox.text('Loading ' + percent + '%');
			}, this);
			...
			$.ajax({
				...
				success: $.proxy(function(data)
				{
					if (this.uploadOptions.success !== false)
					{
						this.uploadOptions.success(data);
					}
		...
		init: function()
		{
			if (!$.browser.opera && !$.browser.msie)
			{
				...
				this.dropareabox.get(0).ondrop = $.proxy(function(event)
				{
					...
					// multiple files upload
					var l = event.dataTransfer.files.length;
					for (var i = 0; i < l; i++) {
